...
const suggestedCoursesModal = document.getElementById("suggested-courses-modal")
...
function switchTab(tab) {
  // Update active tab
  document.getElementById("scheduler-nav").classList.toggle("active", tab === "scheduler")
  document.getElementById("progress-nav").classList.toggle("active", tab === "progress")
...
function showSuggestedCoursesModal() {
  ...
  suggestedCourseCodes.forEach((code) => {
    ...
    if (possibleSections.length > 0) {
      ...
      const hasTimeConflict = !nonConflictingSection && possibleSections.length > 0
      ...
      courseItem.className = `suggested-course-item ${getDepartmentClass(bestSection.Class)}`
      ...
      courseItem.innerHTML = `
        <div class="suggested-course-info">
          <div class="suggested-course-title">${bestSection.Class} ${bestSection.Section}</div>
        ...
        <div class="suggested-course-action">
          <button onclick="addSuggestedCourse(${suggestedCourses.length - 1})" class="primary-button" ${hasTimeConflict ? "disabled" : ""}>
...
function addSuggestedCourse(index) {
  ...
  if (course.hasConflict) {
    showNotification(`Cannot add ${course.Class} ${course.Section}: Time conflict with existing courses.`, "warning")
    return
  }
...
function addAllSuggestedCourses() {
  ...
  if (addedCount > 0) {
    showNotification(`Added ${addedCount} course(s) to your schedule.`, "success")
  ...
  if (conflictCount > 0) {
    showNotification(`${conflictCount} course(s) had time conflicts and were not added.`, "warning")
  }
...
function toggleSearchPopup() {
  ...
  if (isSearchPopupOpen) {
    ...
    searchResults.innerHTML = '<p class="empty-search">Enter a search term to find courses.</p>'
  } else {
...
function searchCourses() {
  ...
  if (currentSearchResults.length === 0) {
    searchResults.innerHTML = '<p class="empty-search">No courses found matching your search.</p>'
  } else {
    searchResults.innerHTML = ""

    currentSearchResults.forEach((course, index) => {
      const resultItem = document.createElement("div")
      resultItem.className = `search-result-item ${getDepartmentClass(course.Class)}`

      resultItem.innerHTML = `
                <div class="search-result-info">
                    <div class="search-result-title">${course.Class} ${course.Section}</div>
...
function removeCourse(courseId) {
  ...
  if (courseToRemove) {
    ...
    showNotification(`Removed ${courseToRemove.Class} ${courseToRemove.Section}`, "normal")
  ...
  let calendarHTML = `
    <div class="weekly-calendar">
      <div class="time-column">
        ${timeMarkers.map((time) => `<div class="time-marker">${time}</div>`).join("")}
      </div>
  `
  ...
  DAYS_OF_WEEK.forEach((day) => {
      ...
      <div class="day-column">
        ...
        <div class="day-schedule">
          ${Array.from({ length: 12 })
            .map((_, i) => `<div class="hour-marker" style="top: ${i * HOUR_HEIGHT}px;"></div>`)
            .join("")}
    `
    ...
    coursesToRender.forEach((course) => {
      if (!course.DaysTimes) {
        ...
        if (day === "Monday") {
          calendarHTML += `
            <div class="course-slot ${getDepartmentClass(course.Class)} ${courseNotes[course.id] ? "has-notes" : ""}"
                 style="top: 0; height: 60px;"
                 onclick="showCourseDetails('${course.id}')">
              <div class="course-code">${course.Class} ${course.Section} (TBA)</div>
      ...
      const shouldDisplayOnDay = matchedDays.some((d) => DAY_MAP[d.toLowerCase()] === day)
      ...
      calendarHTML += `
        <div class="course-slot ${getDepartmentClass(course.Class)} ${courseNotes[course.id] ? "has-notes" : ""}"
          ...
          <div class="course-time">${timeMatch[1]}:${timeMatch[2]} ${timeMatch[3]}-${timeMatch[4]}:${timeMatch[5]} ${timeMatch[6]}</div>
          <div class="course-instructor">${course.Instructor || "N/A"}</div>
          <div class="course-room">${course.Room || "N/A"}</div>
        </div>
      `
    ...
    row.innerHTML = `
      ...
      <td>
        <div class="table-actions">
          <button onclick="showCourseDetails('${course.id}')" class="icon-button" title="View Details">
            <i class="fas fa-info-circle"></i>
          </button>
          <button onclick="openNotesModal('${course.id}')" class="icon-button" title="Add Notes">
            <i class="fas fa-sticky-note"></i>
          </button>
          <button onclick="removeCourse('${course.id}')" class="icon-button" title="Remove Course">
...
function updateStatistics() {
  ...
  const completedCourses = studentCourses.filter((course) => GRADE_POINTS[course.grade] !== null)
...
function renderRequirementsProgress() {
  ...
  studentCourses.forEach((course) => {
    if (course.requirementGroup && REQUIREMENT_GROUPS[course.requirementGroup]) {
      // Only count if not withdrawn and has credits
      if (course.grade !== "WD" && Number.parseFloat(course.credits) > 0) {
        REQUIREMENT_GROUPS[course.requirementGroup].completed += Number.parseFloat(course.credits)
      }
    }
  })
...
function renderProgressDashboard() {
  const dashboardContainer = document.getElementById('progress-dashboard-container');
